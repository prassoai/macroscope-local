...
`--host-permissions grant` lets Claude Code, Cursor, and OpenCode auto-approve Macroscope commands by adding Macroscope/mktemp command allow-rules; Claude Code also receives a narrow `PreToolUse` hook. Skipping it does not disable Macroscope, but those coding agents may ask you to approve each Macroscope command. `--yes` confirms the displayed plan but never implies that permission grant. PATH changes are skipped when `~/.local/bin` is already active and re-evaluated on future updates; otherwise only the login shell's preferred file is changed. Use `--shell-config PATH` for a managed dotfile or `--no-path` to make no shell edits; the installer remembers either explicit choice for future updates.
