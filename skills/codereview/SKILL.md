...
Run this standalone command from the repository being reviewed using the host's background-command support:
...
- Default to the CLI-created review worktree. Only when the user explicitly asks for in-place fixes, pass `--in-place` and apply fixes in their checkout. Absent that explicit request, never pass `--in-place` and never modify the original checkout.
...
- The command blocks. Use the host's native background/async facility; do not use shell redirection, `&`, `nohup`, `tee`, or a separate polling command.
