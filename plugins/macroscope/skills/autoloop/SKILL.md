...
## 2. Run the local CLI review

**Invoke `macroscope codereview` as a standalone command using the host execution mode below.** Keep the review process attached and retain its complete output.
...
- The command blocks. Prefer the host's native background/async facility when it provides a task handle and a way to read/wait on that same task.
- If the host only exposes a foreground shell tool (such as OpenCode's `bash`), run the same standalone command in the foreground with an explicit timeout of at least 1800000 ms (30 minutes). Check the tool's supported limit first; if it cannot keep the command attached that long, stop and report that host limitation before launching.
- The host captures stdout and stderr. Do not add shell redirection of your own (including `2>&1`), `&`, `nohup`, `tee`, a wrapper script, or a separate polling command; the `cd '<isolate_worktree>' &&` prefix of the later-iteration launch is part of that launch, not an addition. Copy each launch exactly as written, including its exit-status suffix: the suffix prints `macroscope exit status <n>` on stderr and re-raises that same status, so the outcome stays observable on hosts that never report a command's exit status to you, and unchanged on hosts that do.
- A foreground-only tool exposes findings after it returns. Wait for that call, then validate the captured findings one at a time in stream order. If the host truncates output, read its saved full output before deciding whether any findings or completion tokens are missing. Do not claim live triage while a foreground call is running.
- If the process exits or the host times out without complete output and the required completion tokens, report the review as incomplete. Do not automatically launch a duplicate review to recover missing output.
