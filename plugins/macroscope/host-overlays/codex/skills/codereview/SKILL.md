...
4. Apply stageable uncommitted changes in the worktree and commit them as a baseline so that `git diff HEAD` later shows only the review fixes. A non-empty patch can still produce no staged diff (for example, a dirty gitlink); treat that as no baseline instead of attempting an empty commit:
