...
- Issues stream directly from the background `codereview` process on stderr as `issue_event=<json>` lines. Read them via `TaskOutput` with the background task ID — no separate polling command is needed.
...
- Continue reading new `issue_event=` lines until the terminal status appears or the process exits.
